...
export function createAlertWebPage(config, alerts) {
  const content = createAlertWebPageHtml(config, alerts);
  const html = buildPage('Alert Page', '', '', 'Alert page', content);
...
export function createAlertWebPageHtml(config, alerts) {
  return importTemplate('alert.html')
    .replace('{ALERT_HEADER}', createAlertHeaderHtml(config))
    .replace('{ALERT_CONTENT}', createAlertContentHtml(config, alerts));
}
...
export function createAlertHeaderHtml(config) {
  let html = '';
  ...
  html = html + `<span>`;
  html = html + `<b>Alert rules:</b> ${JSON.stringify(config.collection.alerts)}`;
  html = html + `<br>`;
  ...
  if (config.collection.assetsInfo) {
    const ref = config.collection.assetsInfo;
...
export function createAlertContentHtml(config, alerts) {
